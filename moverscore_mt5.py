...
from transformers import AutoTokenizer, AutoModel,MT5ForConditionalGeneration,T5Tokenizer,MT5Model,MT5EncoderModel
...
model = MT5EncoderModel.from_pretrained(model_name, output_hidden_states=True, output_attentions=True)
model.eval()


def truncate(tokens):
    if len(tokens) > tokenizer.model_max_length - 2:
        tokens = tokens[0:(tokenizer.model_max_length - 2)]+["</s>"]
    return tokens
...
def bert_encode(model, x, attention_mask):
    model.eval()
    with torch.no_grad():
        result = model(input_ids=x, attention_mask=attention_mask,output_hidden_states=True)
    ...
    for batch_start in range(0, len(refs), batch_size):
        ...
        for i in range(batch_size):
            ref_ids = [k for k, w in enumerate(ref_tokens[i])
                       ...
                       or w in set(string.punctuation) or len(w)==0]
